...
export async function generateMetadata({ params }: { params: { "category-slug": string } }): Promise<Metadata> {
  ...
  const { data: category } = await supabase.from('categories').select('name, description').eq('slug', params["category-slug"]).single()
  
  if (!category) return {}
  
  return {
    title: `${category.name} | Patshell Trading`,
